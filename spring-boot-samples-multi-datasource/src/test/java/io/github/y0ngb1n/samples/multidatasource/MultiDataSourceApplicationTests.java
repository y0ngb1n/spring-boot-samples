...
public class MultiDataSourceApplicationTests {
  ...
  public void testFooDataSource() throws SQLException {
    ...
    Assert.assertNotNull(fooDataSource.getConnection());
    Assert.assertTrue(fooDataSource.getConnection().toString().contains("jdbc:h2:mem:foo"));
  }

  @Test
  public void testBarDataSource() throws SQLException {
    log.info("bar datasource runtime: {}", barDataSource.getConnection());
    Assert.assertNotNull(barDataSource.getConnection());
    Assert.assertTrue(barDataSource.getConnection().toString().contains("jdbc:h2:mem:bar"));
  }
}
